...
демо: `https://sct-back-demo.topcoder.kz`. С 2026-07-19 фронт разделён на
два проекта на общем бэке: `sct-web` (`client_endpoints`) и `sct-admin`
(`staff_endpoints`) — заметки ниже относятся к обоим.

---

## Новое (13.06.2026, после сверки с `Template (3).yaml`)

Фронт перегенерил типы из присланного `Template (3).yaml`. Что всплыло:

1. **`ordering=preferred_date/_time` — всё ещё HTTP 500** (перепроверено live
   13.06 со staff-токеном). В новой схеме эти поля **указаны как разрешённые**
   в описании `ordering`, но по факту ручка падает. `created_at` = 200.
   Фронт продолжает сортировать колонку «Время» на клиенте.
2. **`bookings/options/` — битые `$ref` в схеме.** Ответ ссылается на
   `StaffBookingServiceStation`, `StaffBookingServicePackageOption`,
   `StaffBookingDefaultServiceOption` — этих схем **нет** в `components/schemas`,
   из-за чего `openapi-typescript` падал на всём файле. Фронт локально
   заменил их на нетипизированный объект, чтобы кодген проходил. Допишите
   сериализаторы/`@extend_schema_field`, тогда уберём патч.
3. **`StaffLogin` — схема-компонент всё ещё `{username}`.** В новой версии
   *пример* ответа богатый (`access`/`refresh`/`user`), но сам компонент
   `StaffLogin` по-прежнему только `username`. Кодген берёт компонент, не
   пример → фронт держит локальный `StaffLoginResponse`. Поправьте сам
   сериализатор ответа.
4. **`discount_type` — подтвердите канон.** OpenAPI `DiscountTypeEnum` =
   `NONE | PERCENT | FIXED`. Фронт раньше ошибочно слал `'AMOUNT'` — исправили
   на `'FIXED'`. Подтвердите, что бэк принимает `FIXED` для «скидка суммой»
   (создание пакета с фикс-скидкой) — у нас не было write-теста на демо.
